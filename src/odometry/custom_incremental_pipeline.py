...
from . import custom_bundle_adjustment
import enlighten
...
def iterative_global_refinement(options, mapper_options, tri_options, mapper, max_cost_change_px=5.0, cam_poses=[None, None]):
...
def initialize_reconstruction(
    controller, mapper, mapper_options, reconstruction, cam_poses=[None, None]
):
    ...
    custom_bundle_adjustment.adjust_global_bundle(
        mapper, mapper_options, options.get_global_bundle_adjustment(), cam_poses=cam_poses
    )
...
def reconstruct_sub_model(controller, mapper, mapper_options, tri_options, reconstruction, next_image_id, sequential=False, run_BA=False, max_cost_change_px=5.0, no_stereo_constraint_in_local_ba=False, cam_poses=[None, None]):
    ...
    if reconstruction.num_reg_images() == 0:
        init_status = initialize_reconstruction(
            controller, mapper, mapper_options, reconstruction, cam_poses
        )
    ...
    while True:
        ...
        for reg_trial in range(len(next_images)):
            ...
            if (
                ...
                break
        if reg_next_success:
            mapper.triangulate_image(tri_options, next_image_id) # options.get_triangulation()
            ...
            if controller.check_run_global_refinement(
                ...
                if run_BA: iterative_global_refinement(options, mapper_options, tri_options, mapper, max_cost_change_px, cam_poses=cam_poses)
                ba_prev_num_points = reconstruction.num_points3D()
                ba_prev_num_reg_images = reconstruction.num_reg_images()
            if options.extract_colors:
                extract_colors(
    ...
    #):
    #    if run_BA:iterative_global_refinement(options, mapper_options, tri_options, mapper, max_cost_change_px)
    return pycolmap.IncrementalMapperStatus.SUCCESS


def reconstruct(controller, mapper_options, tri_options, next_image_id, sequential=False, run_BA=False, max_cost_change_px=5.0, no_stereo_constraint_in_local_ba=False, cam_poses=[None, None]):
    ...
    for num_trials in range(options.init_num_trials):
        ...
        status = reconstruct_sub_model(
            controller, mapper, mapper_options, tri_options, reconstruction, next_image_id, sequential, run_BA, max_cost_change_px, no_stereo_constraint_in_local_ba, cam_poses
        )
        # Alternative use
